...
-- 2. Drop the old single-stage RPC (if it exists from the old 07)
...
BEGIN
    ...
    IF v_account_status = 'deleted' THEN
        -- If we reserved a new record, we must fail it since account is already deleted
    ...
    -- 3. Return processing to indicate lock acquired
...
BEGIN
    ...
    v_response_body := jsonb_build_object('success', true, 'data', jsonb_build_object('account', jsonb_build_object('status', 'deleted')));
